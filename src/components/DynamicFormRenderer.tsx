...
const DynamicFormRenderer: React.FC<DynamicFormRendererProps> = ({
  ...
  const renderField = (
    ...
    const baseInputClass =
      "w-full px-3 py-2.5 border border-slate-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all";
            ...
            <div className="flex flex-wrap gap-2">
              {field.options?.map((opt) => {
                ...
                return (
                  <label
                    key={opt}
                    className={`inline-flex items-center gap-2 px-3 py-2 border rounded-xl cursor-pointer transition-all text-sm ${checked ? "bg-indigo-50 border-indigo-300 text-indigo-700" : "bg-white border-slate-300 text-slate-700 hover:bg-slate-50"}`}
                  >
                    <input
                      type="checkbox"
                      checked={checked}
                    ...
                    />
                    <div
                      className={`w-4 h-4 rounded border-2 flex items-center justify-center ${checked ? "bg-indigo-600 border-indigo-600" : "border-slate-300"}`}
                    >
                      {checked && (
                        <svg
