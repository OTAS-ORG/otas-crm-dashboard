...
const ProjectDetail: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col md:flex-row gap-4 sm:gap-0 sm:items-center justify-between">
        <div className=" flex items-center gap-4">
          <button
            onClick={() => navigate("/projects")}
            className="hidden md:block p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl transition-colors"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {COLUMNS.map((column) => {
          ...
          return (
            <div
              key={column.key}
              className="bg-slate-50/70 rounded-2xl border border-slate-200 p-4"
            >
              {/* Column Header */}
                ...
                {columnTasks.map((task) => (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <h4 className="text-sm font-medium text-slate-800 leading-snug flex-1">
                        {task.title}
                      </h4>
                      {task.priority !== "normal" && (
                        <span
                          className={`shrink-0 px-2 py-0.5 rounded-md text-[10px] font-semibold uppercase tracking-wider border ${PRIORITY_STYLES[task.priority]}`}
                        >
                          {task.priority}
                        </span>
                      )}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {task.due_date && (
                        <div
                          className={`flex items-center gap-1 text-xs ${isOverdue(task.due_date) && task.status !== "done" ? "text-red-500" : "text-slate-400"}`}
                        >
                      ...
                      <div className="relative">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setStatusDropdown(
                              statusDropdown === task._id ? null : task._id,
                            );
                          }}
                            ...
                            <div className="absolute right-0 top-full mt-1 z-20 bg-white rounded-xl border border-slate-200 shadow-xl py-1.5 min-w-[140px]">
                              {COLUMNS.map((col) => (
                                <button
                                  key={col.key}
                                  onClick={() =>
                                    handleStatusChange(task._id, col.key)
                                  }
                        ...
                        <div className="flex items-center gap-1">
                          <div className="w-5 h-5 rounded-full bg-emerald-100 flex items-center justify-center">
                            <span className="text-[9px] font-bold text-emerald-600">
                              {getInitials(task.qaAssignedTo)}
                            </span>
                          </div>
                          <span className="text-[10px] text-slate-500">
                            {typeof task.qaAssignedTo === "object"
                              ? task.qaAssignedTo.username
                              : ""}
                          </span>
                        </div>
                      )}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewTask({ ...newTask, qaAssignedTo: e.target.value })
                    }
                    className="w-full px-3 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                  >
                    <option value="">Not assigned</option>
